function showPieChart(divID,dataset){
...
var svg = d3.select('#'+divID) // select element in the DOM with id 'chart'
...
// define tooltip
var tooltip = d3.select("#"+divID) // select element in the DOM with id 'chart'
...
*/ tooltip.html((d.data.label)+"<br>"+(d.data.value)+"<br>"+(percent + '%'));
...
/*path.on("click", function (d) {
	...
	fnOpenScreenPie(d.data.filter,divID);
});*/
...
/*function fnOpenScreenPie(filter,divID){
	if(filter==null || filter == "" || filter == undefined || filter =='undefined'){
		return;
	}
	if(divID=="pieDivIcsr"){
		fnOpenSelectedSectionOpt('esmCaseListingFilter',filter);
	}else if(divID=="pieDivAck"){
		fnOpenSelectedAckOpt('esmCaseListingFilter',filter);
	}
